...
#INPUT_FILE_PATH = 'E:/Fall 2016/Grader/CSE512/Assignment 1/graderData.dat'
INPUT_FILE_PATH = 'graderData.dat'
ACTUAL_ROWS_IN_INPUT_FILE = 2500  # Number of lines in the input file
...
def testloadratings(ratingstablename, filepath, openconnection, rowsininpfile):
    ...
    with openconnection.cursor() as cur:
        cur.execute('SELECT COUNT(*) from public.{0}'.format(RATINGS_TABLE))
        count = int(cur.fetchone()[0])
...
def testroundrobinpartition(ratingstablename, numberofpartitions, openconnection, robinpartitiontableprefix,
    ...
    try:
        ...
        # ignore any exceptions raised by function
        pass
    print("Round Robin Partitions Start Index {}".format(partitionstartindex))
...
def before_test_script_starts_middleware(openconnection, databasename):
    MyAssignment.before_test_script_starts_middleware(openconnection,databasename)
    #pass

def after_test_script_ends_middleware(openconnection, databasename):
    # Use it if you want to
    MyAssignment.after_test_script_ends_middleware(openconnection,databasename)
    #pass
...
if __name__ == '__main__':
    try:
        ...
        with getopenconnection(dbname=DATABASE_NAME) as conn:
            ...
            # ALERT:: Use only one at a time i.e. uncomment only one line at a time and run the script
            testrangeinsert(RATINGS_TABLE, 1, 377, 3.5, conn, RANGE_TABLE_PREFIX + '3')
            ...
            # ALERT:: Use only one at a time i.e. uncomment only one line at a time and run the script
            testroundrobininsert(RATINGS_TABLE, 1, 377, 3.5, conn, RROBIN_TABLE_PREFIX + '0')
